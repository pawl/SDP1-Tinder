    ...
    with settings():
        ...
        with cd(DJANGO_PROJECT_PATH):
            ...
            if not only_static:
                ...
                blacklist = (
                    'scripts',
                    # '%s/settings/*_private.py' % (DJANGO_PROJECT_NAME),
                )
    ...
    with shell_env(**env_variables):
        with cd(DJANGO_PROJECT_PATH):
            if reset_db:
                ...
                with settings(warn_only=True):
                    sudo("chmod -R 700 app") # rwx --- --- : django can write new migrations
...
def _get_private_settings_file(deploy_to=DEFAULT_DEPLOY_TO, local=True):
    ...
    if local:
        ...
        private_file = os.path.join(django_path, DJANGO_PROJECT_NAME,
